...
//For accel setup
   ...
   extern void drawBlast(int16 x,int16 y,int16 size,int32 color,int16 starArray[]);
...
CY_ISR(tapInterrupt_Interrupt)
{
    ...
    //Send the code to get the tap
    WriteBuffer[0]=0x30;
    WriteSensor(WriteBuffer,1);
    ReadSensor(ReadBuffer,1);
    //shift the readbuffer into int
    int tap=ReadBuffer[0]>>5;
    tap=tap&0b011; // and with the binary to make into int 
    ...
    //Hyper blast
    if(tap==3)
           drawBlast(x,y,10,GLCD_BLUE,stars);
