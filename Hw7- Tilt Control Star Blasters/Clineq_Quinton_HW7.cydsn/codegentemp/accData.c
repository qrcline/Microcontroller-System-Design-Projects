...
CY_ISR(accData_Interrupt)
{
    ...
    //Read acc data
      ...
      ReadSensor(ReadBuffer,6);   // Reads DataX0 through Data Z1
       ...
       int16 xData= ((ReadBuffer[0]<<8 )| ReadBuffer[1]);
       int16 yData= ((ReadBuffer[2]<<8) | ReadBuffer[3]);
       int16 zData= ((ReadBuffer[4]<<8 )| ReadBuffer[5]);
        ...
        GLCD_PrintString(stringX,50,30,GLCD_BLUE,GLCD_BLACK);
        sprintf(stringZ, "Z:    %5d",zData);
        GLCD_PrintString(stringX,70,30,GLCD_BLUE,GLCD_BLACK);
        
        
        
      
       
// Accel data is now in ReadBuffer[0], ReadBuffer[1], … , ReadBuffer[5]
