...
#include "D.h"
...
#if defined(D__PC)
    #define D_SetP4PinDriveMode(shift, mode)  \
    do { \
        D_PC =   (D_PC & \
                                (uint32)(~(uint32)(D_DRIVE_MODE_IND_MASK << \
                                (D_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (D_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define D_SetP4PinDriveMode(shift, mode)  \
        do { \
            D_USBIO_CTRL_REG = (D_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(D_DRIVE_MODE_IND_MASK << \
                                    (D_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (D_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		D_SetP4PinDriveMode(D__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(D_DR & (uint8)(~D_MASK));
    drVal = (drVal | ((uint8)(value << D_SHIFT) & D_MASK));
    D_DR = (uint32)drVal;
}
...
uint8 D_Read(void)
{
    return (uint8)((D_PS & D_MASK) >> D_SHIFT);
}
...
{
    return (uint8)((D_DR & D_MASK) >> D_SHIFT);
}
...
void D_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  D_INTCFG & (uint32)(~(uint32)position);
    D_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(D_INTSTAT & D_MASK);
	D_INTSTAT = maskedStatus;
    return maskedStatus >> D_SHIFT;
}
