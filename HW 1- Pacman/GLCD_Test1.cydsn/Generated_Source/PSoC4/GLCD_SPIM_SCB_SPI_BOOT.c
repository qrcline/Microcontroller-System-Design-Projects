...
void GLCD_SPIM_SCB_SpiCyBtldrCommReset(void)
{
    ...
    GLCD_SPIM_SCB_SpiUartClearRxBuffer();
    GLCD_SPIM_SCB_SpiUartClearTxBuffer();
}
...
cystatus GLCD_SPIM_SCB_SpiCyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        {
            /* Check packet start */
            if (0u != GLCD_SPIM_SCB_SpiUartGetRxBufferSize())
            {
                /* Wait for packet end */
                do
                {
                    byteCount = GLCD_SPIM_SCB_SpiUartGetRxBufferSize();
                    CyDelayUs(GLCD_SPIM_SCB_SPI_BYTE_TO_BYTE);
                }
                while (byteCount != GLCD_SPIM_SCB_SpiUartGetRxBufferSize());

                /* Disable data reception into RX FIFO */
                GLCD_SPIM_SCB_RX_FIFO_CTRL_REG |= GLCD_SPIM_SCB_RX_FIFO_CTRL_FREEZE;

                byteCount = GLCD_SPIM_SCB_BYTES_TO_COPY(byteCount, size);
            ...
            CyDelay(GLCD_SPIM_SCB_WAIT_1_MS);
            --timeoutMs;
        }
        while (0u != timeoutMs);
        ...
        {
            pData[i] = (uint8) GLCD_SPIM_SCB_SpiUartReadRxData();
        }
    }
...
cystatus GLCD_SPIM_SCB_SpiCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        GLCD_SPIM_SCB_SpiUartPutArray(pData, (uint32) size);

        /* Wait with timeout 1mS for packet end */
        do
        {
            /* Check for packet end */
            if ((0u == GLCD_SPIM_SCB_SpiUartGetTxBufferSize()) &&
                (0u == (GLCD_SPIM_SCB_SPI_STATUS_REG & GLCD_SPIM_SCB_SPI_STATUS_BUS_BUSY)) &&
                (0u == (GLCD_SPIM_SCB_TX_FIFO_STATUS_REG & GLCD_SPIM_SCB_TX_FIFO_SR_VALID)))
            {
                *count = size;
                status = CYRET_SUCCESS;
        ...
        GLCD_SPIM_SCB_SpiUartClearRxBuffer();
        GLCD_SPIM_SCB_RX_FIFO_CTRL_REG &= (uint32) ~GLCD_SPIM_SCB_RX_FIFO_CTRL_FREEZE;
    }
