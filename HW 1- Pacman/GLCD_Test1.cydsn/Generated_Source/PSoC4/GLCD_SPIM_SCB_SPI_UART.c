...
***************************************/
    ...
    */
    ...
    volatile uint8 GLCD_SPIM_SCB_rxBufferInternal[GLCD_SPIM_SCB_INTERNAL_RX_BUFFER_SIZE];
#endif /* (GLCD_SPIM_SCB_INTERNAL_RX_SW_BUFFER) */

#if(GLCD_SPIM_SCB_INTERNAL_TX_SW_BUFFER)
    volatile uint8 GLCD_SPIM_SCB_txBufferInternal[GLCD_SPIM_SCB_TX_BUFFER_SIZE];
    ...
    *
    * \globalvars
    *  GLCD_SPIM_SCB_rxBufferHead - the start index to put data into the 
    *  software receive buffer.
    *  GLCD_SPIM_SCB_rxBufferTail - the start index to get data from the 
    *  software receive buffer.
    *
    *******************************************************************************/
    uint32 GLCD_SPIM_SCB_SpiUartReadRxData(void)
    {
        ...
        #if (GLCD_SPIM_SCB_CHECK_RX_SW_BUFFER)
        {
            if (GLCD_SPIM_SCB_rxBufferHead != GLCD_SPIM_SCB_rxBufferTail)
            {
                ...
                if (GLCD_SPIM_SCB_INTERNAL_RX_BUFFER_SIZE == locTail)
                {
                    locTail = 0u;
                }
                ...
                #if (GLCD_SPIM_SCB_CHECK_UART_RTS_CONTROL_FLOW)
                {
                    /* Check if RX Not Empty is disabled in the interrupt */
                    if (0u == (GLCD_SPIM_SCB_INTR_RX_MASK_REG & GLCD_SPIM_SCB_INTR_RX_NOT_EMPTY))
                    {
                        /* Enable RX Not Empty interrupt source to continue
                        * receiving data into software buffer.
                        */
                        GLCD_SPIM_SCB_INTR_RX_MASK_REG |= GLCD_SPIM_SCB_INTR_RX_NOT_EMPTY;
                    }
                }
                #endif
        ...
        #else
        {
            ...
            rxData = GLCD_SPIM_SCB_RX_FIFO_RD_REG;
        }
        #endif
    ...
    *******************************************************************************/
    uint32 GLCD_SPIM_SCB_SpiUartGetRxBufferSize(void)
    {
        ...
        #if (GLCD_SPIM_SCB_CHECK_RX_SW_BUFFER)
        {
            ...
            {
                size = (locHead + (GLCD_SPIM_SCB_INTERNAL_RX_BUFFER_SIZE - GLCD_SPIM_SCB_rxBufferTail));
            }
        }
        #else
        {
    ...
    *
    * \globalvars
    *  GLCD_SPIM_SCB_txBufferHead - the start index to put data into the 
    *  software transmit buffer.
    *  GLCD_SPIM_SCB_txBufferTail - start index to get data from the software
    *  transmit buffer.
    *
    *******************************************************************************/
    void GLCD_SPIM_SCB_SpiUartWriteTxData(uint32 txData)
    {
        ...
        #if (GLCD_SPIM_SCB_CHECK_TX_SW_BUFFER)
        {
            ...
            if ((GLCD_SPIM_SCB_txBufferHead == GLCD_SPIM_SCB_txBufferTail) &&
                (GLCD_SPIM_SCB_SPI_UART_FIFO_SIZE != GLCD_SPIM_SCB_GET_TX_FIFO_ENTRIES))
            {
                /* TX software buffer is empty: put data directly in TX FIFO */
                GLCD_SPIM_SCB_TX_FIFO_WR_REG = txData;
            }
            ...
            else
            {
                ...
                GLCD_SPIM_SCB_ClearTxInterruptSource(GLCD_SPIM_SCB_INTR_TX_NOT_FULL);

                GLCD_SPIM_SCB_PutWordInTxBuffer(locHead, txData);

                GLCD_SPIM_SCB_txBufferHead = locHead;

                /* Check if TX Not Full is disabled in interrupt */
                if (0u == (GLCD_SPIM_SCB_INTR_TX_MASK_REG & GLCD_SPIM_SCB_INTR_TX_NOT_FULL))
                {
                    /* Enable TX Not Full interrupt source to transmit from software buffer */
                    GLCD_SPIM_SCB_INTR_TX_MASK_REG |= (uint32) GLCD_SPIM_SCB_INTR_TX_NOT_FULL;
                }
            }
        }
        #else
        {
            ...
            while (GLCD_SPIM_SCB_SPI_UART_FIFO_SIZE == GLCD_SPIM_SCB_GET_TX_FIFO_ENTRIES)
            {
            }
    ...
    *******************************************************************************/
    void GLCD_SPIM_SCB_SpiUartClearTxBuffer(void)
    {
        #if (GLCD_SPIM_SCB_CHECK_TX_SW_BUFFER)
        {
            ...
            GLCD_SPIM_SCB_ClearTxInterruptSource(GLCD_SPIM_SCB_INTR_TX_ALL);
